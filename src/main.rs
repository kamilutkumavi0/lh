...
fn main() {
    ...
    if parsed_args.recursive {
        let mut files = get_files_recursive(conf_hash.clone(), parsed_args.path.clone());
        let filtered_files = filter(&parsed_args, files);
        output_print_recursive(&parsed_args, filtered_files);
    } else {
        let mut files = get_files(conf_hash.clone(), parsed_args.path.clone());
